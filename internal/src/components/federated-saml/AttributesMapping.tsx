      ...
      {mappings.length > 0 && (
        <div className='flex space-x-20 items-center pb-2'>
          <label className='label font-semibold'>
            <span className='label-text'>{t('service-provider-attribute')}</span>
          </label>
          <label className='label font-semibold'>
            <span className='label-text'>{t('identity-provider-attribute')}</span>
          </label>
        </div>
      )}
      <div className='flex flex-col gap-4'>
        ...
        <div>
          <button className='btn btn-primary btn-sm btn-outline' type='button' onClick={addAnother}>
            {mappings.length === 0 ? t('add-mapping') : t('add-another')}
          </button>
        </div>
      </div>
    </div>
