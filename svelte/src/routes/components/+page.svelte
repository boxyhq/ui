<script>
	...
	// @ts-ignore
	async function onSubmitButtonFail({ ssoIdentifier, cb }) {
		cb({
			error: {
				message: 'Invalid team domain'
			}
	...
	// @ts-ignore
	async function onSubmitButton({ cb }) {
	...
	<div class="hero-body">
		<div class="login-with-custom-styles">
			<Login
				...
				inputLabel={'Team Domain *'}
				buttonText={'Login with SSO'}
				innerProps={{ input: { id: 'demo1' }, label: { for: 'demo1' } }}
			/>
			<h2 class="custom-styles-title">Login with custom styling</h2>
			<h2 class="custom-styles-description">
				Refer the code below to see the passed props. Also supported is the passing of style
				attribute for each inner element (Note that inline style will override other styles).
			</h2>
			<button class="btn-toggle" on:click={() => (customStylesIsActive = !customStylesIsActive)}
				>Hide code</button
			>
		...
		<div class="login-with-default-styles">
			...
			<h2 class="default-styles-description">
				If classNames prop is not passed in, then default styling will be applied. Also supported is
				the passing of style attribute for each inner element (Note that inline style will override
				other styles).
			</h2>
			<button class="btn-toggle" on:click={() => (defaultStylesIsActive = !defaultStylesIsActive)}
				>Hide code</button
			>
		...
		<div class="login-without-input">
			...
			<h2 class="login-without-input-title-description">
				Here we pass the ssoIdentifier directly instead of taking a user input.
			</h2>
			<button class="btn-toggle" on:click={() => (withoutInputIsActive = !withoutInputIsActive)}
				>Hide code</button
			>
		...
		<div class="login-with-failing-onsubmit">
			...
			<button
				class="btn-toggle"
				on:click={() => (withFailingOnsubmitIsActive = !withFailingOnsubmitIsActive)}
				>Hide code</button
			>
...
<style global>
	...
	.login-with-custom-styles,
	.login-with-default-styles,
	.login-without-input,
	.login-with-failing-onsubmit {
		border: 1px solid rgba(5, 5, 5, 0.1);
		padding: 20px;
		margin: 15px 0;
	}

	.login-with-custom-styles h2,
	.login-with-default-styles h2,
	.login-without-input h2,
	.login-with-failing-onsubmit h2 {
		font-size: 16px;
		font-weight: normal;
	}
	...
	.login-without-input-title,
	.login-with-failing-onsubmit-title {
		margin: 10px 0;
	...
	.login-without-input-title-description,
	.login-with-failing-onsubmit-description {
