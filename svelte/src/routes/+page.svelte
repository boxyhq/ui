	...
	<div class="hero-body">
		...
		<p class="description">
			UI components from <span class="boxy-link"
				><a href="https://boxyhq.com/" target="_blank">BoxyHQ</a></span
		...
		<p>
			If a value is passed for <code class="code">ssoIdentifier</code>, it would render a button
			that on click calls the passed-in handler (onSubmit) with the
			<code class="code">ssoIdentifier</code> value. The handler can then initiate a redirect to the
			SSO service forwarding the value for ssoIdentifier.
		</p>
		...
		<h4 class="preset-value">
			Accept input from the user for <code class="code">ssoIdentifier</code>
		</h4>
		<p>
			If a value is not passed for <code class="code">ssoIdentifier</code>, it would render an input
			field for the user to enter the <code class="code">ssoIdentifier</code> value. And then on submit,
			the value gets passed to the handler. The handler can then initiate a redirect to the SSO service
			forwarding the value for ssoIdentifier.
		</p>
		<div class="code-to-be-highlighted">
			<code>
		...
		<p>
			If the classNames prop is passed in, we can override the default styling for each inner
			element. In case an inner element is omitted from the classNames prop, default styles will be
			set for the element. For example, In the below snippet, all the inner elements are styled by
			passing in the classNames for each inner one.
		</p>
		<div class="code-to-be-highlighted">
			<code>
